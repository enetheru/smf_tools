...
struct Arg: option::Arg
{
    inline static bool werror = false;
    inline static SMFHeader smf_h;
    ...
    static option::ArgStatus MapX( const option::Option& option, bool msg ) {
        smf_h.mapx = std::stoi( option.arg );
        if( smf_h.mapx < 128 ){
            SPDLOG_CRITICAL("mapx value needs to be at least 128");
            return option::ARG_ILLEGAL;
        }
        if( smf_h.mapx % 128 ){
            SPDLOG_CRITICAL("mapx value needs to be a multiple of 128");
            return option::ARG_ILLEGAL;
        }
        if( smf_h.mapx > 8192 ){
            SPDLOG_WARN("mapx values larger than {} will likely crash the engine", 8192);
            if( werror ) return option::ARG_ILLEGAL;
        }
        return option::ARG_OK;
    }
};
